...
def appStarted(app):
    ...
    with open(app.logPath, 'r') as f:
        ...
        for i, r in enumerate(cR):
            app.data[i] = r
    app.currSample = 0
    app.drawReal = app.getUserInput("Draw Real NBA Locations? (y/n)") == 'y'
...
def keyPressed(app, event):
    if event.key=="Left":
        app.currSample = max(0, app.currSample-20)
    if event.key=="Right":
        app.currSample = min(len(app.data.values()), app.currSample+20)
...
def redrawAll(app, canvas):
    ...
    for idx in range(1+app.currSample, min(len(app.data.values()), 1+app.currSample+draw_sample_window)):
        if app.drawReal:
            prevX, prevY = footToPixel(app, float(app.data[idx-1]["x_loc"]), float(app.data[idx-1]["y_loc"]))
            currX, currY = footToPixel(app, float(app.data[idx]["x_loc"]), float(app.data[idx]["y_loc"]))
            canvas.create_line(prevX, prevY, currX, currY, fill=realColor, activefill="red", width=1)
